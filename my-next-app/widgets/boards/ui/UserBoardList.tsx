...
import { BoardItem } from "@/shared/ui/board/board.type";
...
export default function UserBoardList({
  ...
  userId: string;
  initialItems: UserBoardListItem[];
  initialCursor: string | null;
  initialHasNext: boolean;
}) {
  const { items, loading, errorMsg, hasNext, sentinelRef } =
    useInfiniteScrollList<UserBoardListItem>({
      initialItems,
      initialCursor,
      initialHasNext,
      limit: 6,
      ...
      fetchPage: async ({ cursor, limit }) => {
        ...
        return {
          items: result.items as UserBoardListItem[],
